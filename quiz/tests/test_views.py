...
class QuizTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.assessment = Assessment.objects.create(
            user=cls.user, quiz=cls.quiz, score=0)
    ...
    def test_edit_question_in_quiz_view(self):
        ...
        response = self.client.post(
            f'/quizzes/{self.quiz.slug}/{self.question.pk}/edit_question/')
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_option_in_quiz_view(self):
        ...
        response = self.client.get(
            f'/quizzes/{self.quiz.slug}/{self.option.pk}/edit_option/')
        self.assertEqual(response.status_code, 200)

    def test_delete_question_in_quiz_view(self):
        """Test get success url in DeleteQuestionInQuizView"""
        response = self.client.post(
            f'/quizzes/{self.quiz.slug}/{self.question.pk}/delete_question/')
    ...
    def test_delete_option_in_quiz_view(self):
        ...
        response = self.client.post(
            f'/quizzes/{self.quiz.slug}/{self.option.pk}/delete_option/')
        self.assertRedirects(
            response, f'/quizzes/test-quiz/{self.question.pk}/details/')
